...
func TestSecurityMiddleware(t *testing.T) {
	for _, test := range []struct {
		...
		{"shadow bans", "GET", "/api/admin/shadow-bans", "", "Bearer secret", false, 204},
		{"shadow ban update", "POST", "/api/admin/shadow-bans", `{"userId":"123","banned":true}`, "Bearer secret", false, 204},
		{"pixel owner", "GET", "/api/admin/boards/main/pixel?x=1&y=2", "", "Bearer secret", false, 204},
		...
		{"board privacy requires auth", "GET", "/api/boards/profiles/me/privacy", "", "", false, 401},
		{"board privacy update requires auth", "PUT", "/api/boards/profiles/me/privacy", `{\"hideUsername\":true}`, "", false, 401},
		{"board privacy rejects post", "POST", "/api/boards/profiles/me/privacy", `{}`, "", false, 405},
